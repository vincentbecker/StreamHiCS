...
public class KolmogorovSmirnov extends StatisticalTest {
	...
	public KolmogorovSmirnov() {
		...
		/*
		 * int expectedSampleSize = (int) alpha * numberOfInstances; dMin =
		 * calculateExpectedDMin(numberOfInstances, expectedSampleSize, 100);
		 * dMax = calculateExpectedDMax(numberOfInstances, expectedSampleSize);
		 */
	}

	/*
	 * private double calculateExpectedDMin(int numberOfInstances, int
	 * expectedSampleSize, int iterations) { double totalDev = 0.0; var
	 * selection = newIndexSelection(N) for(int i = 0; i < iterations; i++){
	 * selection.selectRandomly(M) totalDev +=
	 * computeDeviationFromSelfSelection(selection); } return totalDev /
	 * iterations; }
	 * 
	 * private double calculateExpectedDMax(int numberOfInstances, int
	 * expectedSampleSize) { double totalDeviation = 0.0; var selection =
	 * newIndexSelection(N) let possibleOffsets = selection.possibleOffsets(M)
	 * for offset in possibleOffsets.min .. possibleOffsets.max:
	 * selection.selectBlock(M, offset) totalDeviation +=
	 * computeDeviationFromSelfSelection(selection) return totalDeviation /
	 * possibleOffsets.numPossible }
	 */
