...
public class TreeCoreset {
	...
	Point chooseCentre(treeNode node, MTRandom clustererRandom) {
		...
		for (j = 0; j < times; j++) {
			...
			for (i = 0; i < node.n; i++) {
				...
				if (sum >= random) {
					if (node.points[i].weight == 0.0) {
						// printf("ERROR: CHOOSEN DUMMY NODE THOUGH OTHER
						// AVAILABLE \n");
						return null;
					}
	...
	void split(treeNode parent, Point newCentre, int newCentreIndex) {
		...
		// 1. Counts how many points belong to the new and how many points
		// belong to the old centre
		int nOld = 0;
		int nNew = 0;
		...
		/*
		 * lc.centre = parent.centre; lc.points = oldPoints; lc.n = nOld;
		 * 
		 * lc.lc = null; lc.rc = null; lc.parent = parent;
		 * 
		 ...
		 * 
		 * rc.lc = null; rc.rc = null; rc.parent = parent;
		 * 
	...
	void unionTreeCoreset(int k, int n_1, int n_2, int d, Point[] setA, Point[] setB, Point[] centres,
