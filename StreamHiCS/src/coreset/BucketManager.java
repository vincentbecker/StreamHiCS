	...
	public void insertPoint(Point p) {
		...
		if (cursize >= this.maxBucketsize) {
			...
			if (this.buckets[nextbucket].cursize == 0) {
				...
				nextbucket++;
				/*
				 * as long as the next bucket is full output the coreset to the
				 * spillover of the next bucket
				 */
				while (this.buckets[nextbucket].cursize == this.maxBucketsize) {
					// printf("Bucket %d full \n",nextbucket);
					this.treeCoreset.unionTreeCoreset(this.maxBucketsize, this.maxBucketsize, this.maxBucketsize,
							p.dimension, this.buckets[curbucket].points, this.buckets[curbucket].spillover,
		...
		if (this.buckets[this.numberOfBuckets - 1].cursize == this.maxBucketsize) {
			...
			for (j = i + 1; j < this.numberOfBuckets; j++) {
				if (this.buckets[j].cursize != 0) {
					...
					this.treeCoreset.unionTreeCoreset(this.maxBucketsize, this.maxBucketsize, this.maxBucketsize, d,
							this.buckets[j].points, coreset, this.buckets[j].spillover, this.clustererRandom);
