...
public class ClusTree extends AbstractClusterer {
	...
	private Entry insertBreadthFirst(ClusKernel newPoint, Budget budget, long timestamp) {
		// check all leaf nodes and get the one with the closest entry to
		// newPoint
		...
		// if there is space, add it to the node ( doesn't ever occur, since
		// nodes are created with 3 entries)
		if (numFreeEntries > 0) {
		...
		// if there is space/outdated cluster on path to top, split. Else merge
		// without split
		else {
	...
	private Entry insertHere(Entry newEntry, Node currentNode, Entry parentEntry, ClusKernel carriedBuffer,
	...
	private boolean hasMaximalSize() {
		// TODO: Improve hasMaximalSize(). For now it just works somehow for
		// testing.
	...
	// TODO: Microcluster unter dem Threshhold nich zur�ckgeben (WIe bei
	// outdated entries)
	@Override
